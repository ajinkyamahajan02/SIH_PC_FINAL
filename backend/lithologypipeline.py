...
import pickle
model_class=load_model('SelectiveDataset30.h5')
...
def prediction(image):
  ...
  # print(df_testing)
  pred=svc_model.predict_proba(df_testing)
...
def overall_cylinders(all_images_list,cropped_images):
  ...
  for each_cylinder,each_image in zip(all_images_list,cropped_images):
    ...
    for unit in each_cylinder:
      ...
      if max(predi[0]) >= 0.5:
        pred_class_cnn=np.argmax(predi)
        if pred_class_cnn == 1 or np.argmax(pred_class) == 1:
            counter_d["Sandstone"] += 1
        else:
            predi1 = (pred_class_cnn*(0.7) + pred_class*(0.3))
            final_class = np.argmax(predi1)
            counter_d[d[final_class]] += 1
        # counter_d[d[pred_class]]+=1
        out.append({'class': d[final_class], 'start_point': (cnt * 50, int(400 / n)), 'end_point': (cnt * 50 + 50, 0)})

      else:
        counter_d["Unknown"]+=1
        out.append({'class': 'Unknown', 'start_point': (cnt * 50, int(400 / n)), 'end_point': (cnt * 50 + 50, 0)})
      cnt = cnt + 1
